...
public class RemoveAvengerHandler implements RequestHandler<Avenger, HandlerResponse> {
	...
	public HandlerResponse handleRequest(final Avenger avenger, final Context context) {
		...
		final Optional<Avenger> avengerToDelete = dao.search(id);

		if (avengerToDelete.isPresent()) {

			context.getLogger().log("[#] - Avenger found " + avengerToDelete.get().getName() + " deleting...");
			
			dao.delete(avengerToDelete.get());
